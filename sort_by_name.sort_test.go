...
// go test -v -count=1 ./ -test.run=TestName_Sort
func TestName_Sort(t *testing.T) {
	srcPath := "./testdata"
	...
	var nameSlice = make([]Name, len(fi))
	for i := range fi {
		nameSlice[i] = Name(fi[i])
	}

	// 排序后
	sortNameHandler.Asc(nameSlice)
	sortNameHandler.Desc(nameSlice)
